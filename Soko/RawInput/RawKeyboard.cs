...
namespace RawInput_dll
{
	public sealed class RawKeyboard
	{
        ...
        internal void GetDeviceNameAndVKey(IntPtr hdevice, out string deviceName, out int vkey)
        {
            deviceName = "";
            vkey = -1;
            InputData rawBuffer2;
            if (_deviceList.Count == 0) return;
            ...
            if (dwSize != Win32.GetRawInputData(hdevice, DataCommand.RID_INPUT, out rawBuffer2, ref dwSize,
                Marshal.SizeOf(typeof(Rawinputheader))))
            {
                Debug.WriteLine("Error getting the rawinput buffer2");
                return;
            }

            vkey = rawBuffer2.data.keyboard.VKey;

            // TODO4: Da li treba ova linija (ima je gore u ProcessRawInput)
            //if (vkey == Win32.KEYBOARD_OVERRUN_MAKE_CODE) return;

            KeyPressEvent keyPressEvent;

            if (_deviceList.ContainsKey(rawBuffer2.header.hDevice))
            {
                lock (_padLock)
                {
                    keyPressEvent = _deviceList[rawBuffer2.header.hDevice];
                }
            }
            else
            {
                Debug.WriteLine("Handle: {0} was not in the device list.", rawBuffer2.header.hDevice);
                return;
            }
            deviceName = keyPressEvent.Name + "***" + keyPressEvent.DeviceName + "***" + keyPressEvent.DeviceType
