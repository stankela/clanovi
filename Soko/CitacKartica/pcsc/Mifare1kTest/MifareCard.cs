...
namespace Mifare1kTest {
    public class MifareCard {
        ...
        public byte[] ReadUID()
        {
            var readUIDCmd = new CommandApdu(IsoCase.Case2Short, SCardProtocol.Any)
            {
                CLA = 0xFF,
            ...
            Debug.WriteLine($"Read UID: {BitConverter.ToString(readUIDCmd.ToArray())}");
            var response = _isoReader.Transmit(readUIDCmd);
            Debug.WriteLine($"SW1 SW2 = {response.SW1:X2} {response.SW2:X2} Data: {BitConverter.ToString(response.GetData())}");
